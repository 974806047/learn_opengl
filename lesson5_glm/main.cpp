...
void testCase1() {
    ...
    std::cout<<1<<std::endl;
    InitData initData = init(vertices, 32, indices, 6, true);
    std::cout<<2<<std::endl;
    ...
    while(!glfwWindowShouldClose(initData.window))
    {
        ...
        // ImGui::NewFrame();
        // ImGui::Begin("Another Window", &show_imgui_window);   // Pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
...
void testCase2() {
    ...
    unsigned int texture2 = read_image("./image/laugh.png", GL_RGBA, GL_RGBA);
    // 创建并使用程序对象
    Shader shader("./shader/obj2.vert", "./shader/obj2.frag");
    // glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); // 使用线框模式
    float time = 0;
