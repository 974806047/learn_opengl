...
void testCase1() {
    ...
    GLBufferData objBufferData = initGLBuffer(cubeVertices, 36 * 5, true);
    GLBufferData planeBufferData = initGLBuffer(planeVertices, 6 * 5, true);
    // 创建并使用程序对象
    Shader objShader("./shader/depth.vert", "./shader/depth.frag");
    unsigned int marble = read_image("./image/marble.jpg", GL_RGB, GL_RGB);
...
void testCase2() {
    ...
    GLBufferData planeBufferData = initGLBuffer(planeVertices, 6 * 5, true);
    // 创建并使用程序对象
    Shader objShader("./shader/template1.vert", "./shader/template1.frag");
    Shader lineShader("./shader/template2.vert", "./shader/template2.frag");
    unsigned int marble = read_image("./image/marble.jpg", GL_RGB, GL_RGB);
    unsigned int metal = read_image("./image/metal.png", GL_RGB, GL_RGB);
    // glPolygonMode(GL_FRONT_AND_BACK, GL_LINE); // 使用线框模式
    float time = 0;
    ...
    glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); // 模板测试和深度测试都通过时就将模板值替换成glStencilMask里的值
    // glDepthFunc(GL_ALWAYS); // 深度测试永远通过
    bool show_imgui_window = true;
    float aspect = (float)WIDTH / (float)HEIGHT;
    objShader.use();
...
#pragma endregion
        ...
        // -----------------------------------------------------------------------------------------------------------------------------
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF); // 不等于1才绘制
        glStencilMask(0x00);
